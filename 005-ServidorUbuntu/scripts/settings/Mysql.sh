...
2. New password root: jensy <Enter>
3. Re-enter new password root: jensy <Enter>
...
3. New password: jensy <Enter>
4. Re-enter new password: jensy <Enter>
...
SHOW DATABASES;
#
# Criando o nosso Banco de Dados jensy no SGBD do MySQL ou MariaDB
# Verificando o nosso Banco de Dados criado no SGBD do MySQL ou MariaDB
CREATE DATABASE jensy;
SHOW DATABASES;
#
...
# Primeiro criar o usuário: CREATE USER 'jensy'@'localhost' IDENTIFIED WITH mysql_native_password BY 'jensy';
# Segundo aplicar as permissões: GRANT ALL PRIVILEGES ON *.* TO 'jensy'@'localhost';
# Terceiro aplicar todas as mudanças: FLUSH PRIVILEGES;
GRANT ALL ON *.* TO root@'%' IDENTIFIED BY 'jensy'
#
...
CREATE USER 'jensy' IDENTIFIED BY 'jensy';
#
# Aplicando as permissões de acesso ao Banco de Dados jensy no SGBD do MySQL ou MariaDB
# (opções do comando GRANT: grant (permissão), usage (uso em banco ou tabela), on (em ou na | banco ou tabela), *.* (todos os bancos/tabelas))
# (opções do comando GRANT: to (para), 'jensy' (usuário), identified by (identificado por - senha do usuário))
# (opções do comando GRANT: all (todos privilégios), privileges (privilégios), on (em ou na | banco ou tabela), jensy.* (banco/tabelas), to (para) 'jensy' (usuário))
GRANT USAGE ON *.* TO 'jensy' IDENTIFIED BY 'jensy';
GRANT ALL PRIVILEGES ON jensy.* TO 'jensy';
FLUSH PRIVILEGES;
EXIT
#
# Acessando o SGBD do MySQL ou MariaDB com o usuário jensy
# (opções do comando mysql: -u user | -p password)
mysql -u jensy -p
#
# Utilizando o Banco e Dados jensy no SGBD do MySQL ou MariaDB
SHOW DATABASES;
USE jensy;
#
...
INSERT INTO alunos VALUES ('000001', 'Robson jensy', 'Guarulhos');
